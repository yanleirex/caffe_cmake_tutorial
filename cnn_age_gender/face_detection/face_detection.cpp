...
void face_detecton(const cv::Mat& wildImage, std::vector<BoundingBox>& bdBoxes)
{
    int min_scale = 0;
    int max_scale = 0;
    int factor_count = 0;
    ...
    for(const auto& scaling:scalings)
    {
        ...
        cv::resize(channels[0], blueChannel, cv::Size(scaledCols, scaledRows));
        cv::resize(channels[1], redChannel, cv::Size(scaledCols, scaledRows));
        cv::resize(channels[2], greenChannel, cv::Size(scaledCols, scaledRows));
        ...
        net->CopyTrainedLayersFrom(project_root+"source/face_full_conv.caffemodel");
