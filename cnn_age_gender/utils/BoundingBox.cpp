...
void nms_average(std::vector<BoundingBox>& bd, std::vector<BoundingBox>& processedBox, float overlapped_thresh)
{
...
void nms_max(std::vector<BoundingBox>& bd, std::vector<BoundingBox>& processedBox, float overlapped_thresh)
{
    ...
    for(int i=0;i<bd.size();i++)
    {
        ...
        for(;j<processedBox.size();j++)
        {
            ...
            float x_overlap = MAX(0, MIN(x21, x22)-MAX(x11, x21));
            float y_overlap = MAX(0, MIN(y12, y22)-MAX(y11, y21));
