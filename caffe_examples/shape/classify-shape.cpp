...
int main(int argc, char* argv[])
{
    ...
    for(int i=0;i<3;i++)
    {
        ...
        const int y1 = std::min(std::max(((float)std::rand()/RAND_MAX)*in_image_bgr.rows, (float)20), (float)rows-20);
        const int x2 = x1 + 8;
        const int y2 = y1 + 8;

        cv::rectangle(in_image_bgr, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(0, 0, 255), -1, 8);
    }
    ...
    for(int i=0;i<3;i++)
    {
        ...
        cv::circle(in_image_bgr, cv::Point(x1, y1), 5, cv::Scalar(0, 255, 0), -1, 8);
    }
    ...
    long iCorrectPixelClass0 = 0, iCorrectPixelClass1 = 0, iCorrectPixelClass2 = 0;
    const int kernel = 15;
    const int h_kernel = kernel/2;
    for(int y=h_kernel;y<in_image.rows-h_kernel;y++)
    {
        //create input blob for a whole line which is much faster
        ...
        //mark classification result in outout image
    ...
    std::cout<<"Classified "<< static_cast<double>(iCorrectPixelClass0)/iPixelsClass0*100<<"% correctly in class 0"<<std::endl;
    std::cout<<"Classified "<< static_cast<double>(iCorrectPixelClass1)/iPixelsClass0*100<<"% correctly in class 0"<<std::endl;
    std::cout<<"Classified "<< static_cast<double>(iCorrectPixelClass2)/iPixelsClass0*100<<"% correctly in class 0"<<std::endl;
    std::cout<<"Classified "<< static_cast<double>(iCorrectPixelClass0+iCorrectPixelClass1+iCorrectPixelClass2)/iProcessedPixels*100
