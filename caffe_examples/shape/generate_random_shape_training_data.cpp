...
//define gflags FLAGS and default value
DEFINE_string(backend, "lmdb", "The backend{lmdb, leveldb} for storing the result");
DEFINE_int32(split, 1, "Number of samples{nr}used for TRAIN before a sample is used for TEST, use"
        "negative value to do the opposite");
...
int main(int argc, char* argv[])
{
    ...
    gflags::SetUsageMessage("Generates random training data sample and puts it in\n"
                            "the leveldb/lmdb format used as input for Caffe.\n"
                            "Usage:\n"
                            "generate_random_shape_training_data [FLAGS] DB_NAME");
    ...
    tCounts counts;
    for(tSamples::const_iterator itrSample = samples.begin(); itrSample != samples.end(); ++itrSample)
    {
        counts[itrSample->second]++;
    }

    //show counts
    for(tCounts::const_iterator itr = counts.begin(); itr != counts.end();++itr)
    {
    ...
    eNextSample nextSample = eNSTrain;
    ...
    for(tSamples::const_iterator itrSample = samples.begin();itrSample != samples.end();++itrSample)
    {
        ...
        for(tData::const_iterator itrInputData = itrSample->first.begin();itrInputData != itrSample->first.end();++itrInputData)
        {
        ...
        //determin where next sample should go
        if(iSplitRate == 0)
        {
            nextSample = eNSTrain;
        }
